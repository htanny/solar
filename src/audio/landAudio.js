...
function startLandSound(plName){
  ...
  try{
    ...
    if(plName==="Earth"){
      ...
      /* 小惑星帯最大の天体: ほぼ完全な静寂(大気なし・地質活動ほぼ停止)
         14Hzの微弱な残響 + 稀なマイクロインパクト + 水蒸気昇華のかすかなヒス
         (ハーシェル宇宙望遠鏡が検出した一時的な水蒸気放出を音響化) */
      var oCe=ac.createOscillator();oCe.type="sine";oCe.frequency.value=14;var gCe=ac.createGain();gCe.gain.value=0.08;oCe.connect(gCe);gCe.connect(master);oCe.start();nodes.push(oCe);
      var oCe2=ac.createOscillator();oCe2.type="sine";oCe2.frequency.value=28;var gCe2=ac.createGain();gCe2.gain.value=0.03;oCe2.connect(gCe2);gCe2.connect(master);oCe2.start();nodes.push(oCe2);
      var ceImpInt=setInterval(function(){if(ac.state==="closed")return;var bce=ac.createBufferSource();var bufce=ac.createBuffer(1,Math.floor(ac.sampleRate*0.9),ac.sampleRate);var dce=bufce.getChannelData(0);for(var cei=0;cei<dce.length;cei++)dce[cei]=(Math.random()*2-1)*0.38*Math.exp(-cei/ac.sampleRate*7);bce.buffer=bufce;var gce=ac.createGain();gce.gain.value=0.024;var fce=ac.createBiquadFilter();fce.type="lowpass";fce.frequency.value=65;bce.connect(fce);fce.connect(gce);gce.connect(master);bce.start();},15000+Math.random()*20000);
      var ceSubInt=setInterval(function(){if(ac.state==="closed")return;var bsu=ac.createBufferSource();var bufsu=ac.createBuffer(1,Math.floor(ac.sampleRate*2.0),ac.sampleRate);var dsu=bufsu.getChannelData(0);for(var sui=0;sui<dsu.length;sui++)dsu[sui]=(Math.random()*2-1)*0.25*Math.min(1,sui/(ac.sampleRate*0.6))*Math.exp(-sui/ac.sampleRate*1.4);bsu.buffer=bufsu;var gsu=ac.createGain();gsu.gain.value=0.012;var fsu=ac.createBiquadFilter();fsu.type="highpass";fsu.frequency.value=1400;bsu.connect(fsu);fsu.connect(gsu);gsu.connect(master);bsu.start();},12000+Math.random()*16000);
      LAND_AUDIO={ac:ac,master:master,nodes:nodes,intervals:[ceImpInt,ceSubInt]};
    }else if(plName==="Eris"){
      /* 太陽系最遠の準惑星: ほぼ完全な虚空の沈黙(大気なし・地質活動ゼロ)
         10Hzの極微弱な残響 + 超稀なマイクロインパクト + 微かなメタン昇華ヒス
         (近日点付近でのみ発生するメタン昇華を音響表現) */
      var oEr=ac.createOscillator();oEr.type="sine";oEr.frequency.value=10;var gEr=ac.createGain();gEr.gain.value=0.05;oEr.connect(gEr);gEr.connect(master);oEr.start();nodes.push(oEr);
      var erImpInt=setInterval(function(){if(ac.state==="closed")return;var ber=ac.createBufferSource();var bufer=ac.createBuffer(1,Math.floor(ac.sampleRate*1.2),ac.sampleRate);var der=bufer.getChannelData(0);for(var eri=0;eri<der.length;eri++)der[eri]=(Math.random()*2-1)*0.3*Math.exp(-eri/ac.sampleRate*5);ber.buffer=bufer;var ger=ac.createGain();ger.gain.value=0.016;var fer=ac.createBiquadFilter();fer.type="lowpass";fer.frequency.value=50;ber.connect(fer);fer.connect(ger);ger.connect(master);ber.start();},28000+Math.random()*32000);
      var erSubInt=setInterval(function(){if(ac.state==="closed")return;var bsu2=ac.createBufferSource();var bufsu2=ac.createBuffer(1,Math.floor(ac.sampleRate*1.6),ac.sampleRate);var dsu2=bufsu2.getChannelData(0);for(var sui2=0;sui2<dsu2.length;sui2++)dsu2[sui2]=(Math.random()*2-1)*0.18*Math.min(1,sui2/(ac.sampleRate*0.8))*Math.exp(-sui2/ac.sampleRate*1.0);bsu2.buffer=bufsu2;var gsu2=ac.createGain();gsu2.gain.value=0.006;var fsu2=ac.createBiquadFilter();fsu2.type="highpass";fsu2.frequency.value=1800;bsu2.connect(fsu2);fsu2.connect(gsu2);gsu2.connect(master);bsu2.start();},22000+Math.random()*28000);
      LAND_AUDIO={ac:ac,master:master,nodes:nodes,intervals:[erImpInt,erSubInt]};
