...
function dRiUranus(ctx,wx,wy,wz,pr,cam){var tr=97.8*0.01745;var rings=[{i:1.38,o:1.40,c:"rgba(60,80,90,0.30)"},{i:1.50,o:1.52,c:"rgba(60,80,90,0.28)"},{i:1.58,o:1.60,c:"rgba(70,90,100,0.28)"},{i:1.65,o:1.68,c:"rgba(70,90,100,0.32)"},{i:1.76,o:1.79,c:"rgba(80,100,110,0.28)"},{i:1.90,o:1.95,c:"rgba(70,90,100,0.38)"}];var n=Math.max(24,Math.min(180,Math.floor(pr*2*cam.zm*0.5)));for(var li=0;li<rings.length;li++){var L=rings[li],ot=[],it=[];for(var j=0;j<=n;j++){var a=(j/n)*TAU,ca=Math.cos(a),sa=Math.sin(a);ot.push(pj(wx+ca*pr*L.o,wy+sa*pr*L.o*Math.cos(tr),wz+sa*pr*L.o*Math.sin(tr),cam));it.push(pj(wx+ca*pr*L.i,wy+sa*pr*L.i*Math.cos(tr),wz+sa*pr*L.i*Math.sin(tr),cam));}ctx.beginPath();for(var k=0;k<ot.length;k++){if(k===0)ctx.moveTo(ot[k].x,ot[k].y);else ctx.lineTo(ot[k].x,ot[k].y);}for(var k2=it.length-1;k2>=0;k2--)ctx.lineTo(it[k2].x,it[k2].y);ctx.closePath();ctx.fillStyle=L.c;ctx.fill();}}
...
function drawPlanetBody(ctx,cx,cy,r,pl,rotAngle,cam){
  ...
  var tr=pl.t*0.01745;var ap=cam?RX(RY([Math.sin(tr),Math.cos(tr),0],cam.ry),cam.rx):[Math.sin(tr),Math.cos(tr),0];var screenTilt=Math.atan2(ap[0],ap[1]);
...
export { dOb, dRi, dRiUranus, dSh, dAx, drawPlanetBody, drawSun, mkStars, mkNeb, sSP, mkAst, mkGalaxy, mkNearStars, SD, NB, AST, GAL, GAL_COLS, GAL_R, SUN_GAL_R, SUN_GAL_ANG, NEAR_STARS, SUNSPOTS };
