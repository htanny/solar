...
/**
 * @typedef {{n:string,d:number,r:number,p:number,c:string,t:number}} PlanetData
 * @typedef {{fx:number,fy:number,fz:number,rx:number,ry:number,zm:number}} Camera
 * @typedef {[number,number,number]} Vec3
 */

/** Orbit radius for a planet on screen. @param {PlanetData} p @param {boolean} rd @param {boolean} un @returns {number} */
...
/** Planet radius on screen. @param {PlanetData} p @param {boolean} rp @param {boolean} un @returns {number} */
...
/** Sun radius on screen. @param {boolean} rs @param {boolean} un @returns {number} */
export function sRf(rs,un){if(un)return(SRR/1000)*DK;if(rs)return SRR*SK;return 18;}
/** Moon orbit radius around Earth on screen. @param {boolean} rd @param {boolean} un @returns {number} */
export function mOf(rd,un){if(un)return MD.rd*DK;if(rd)return MD.rd*1000*DK;return MD.oR;}
/** Moon radius on screen. @param {boolean} rp @param {boolean} un @returns {number} */
export function mRf(rp,un){if(un)return Math.max((MD.rr/1000)*DK,0.0001);if(rp)return Math.max(MD.rr*SK,0.5);return MD.r;}

/** Rotation around Y axis. @param {Vec3} p @param {number} a @returns {Vec3} */
export function RY(p,a){var c=Math.cos(a),s=Math.sin(a);return[p[0]*c+p[2]*s,p[1],-p[0]*s+p[2]*c];}
/** Rotation around X axis. @param {Vec3} p @param {number} a @returns {Vec3} */
export function RX(p,a){var c=Math.cos(a),s=Math.sin(a);return[p[0],p[1]*c-p[2]*s,p[1]*s+p[2]*c];}
/** Project 3D world point to screen coordinates. @param {number} x @param {number} y @param {number} z @param {Camera} c @returns {{x:number,y:number,z:number}} */
export function pj(x,y,z,c){var p=RX(RY([x-c.fx,y-c.fy,z-c.fz],c.ry),c.rx);return{x:p[0]*c.zm,y:p[1]*c.zm,z:p[2]};}

/** Set canvas path to a circle (use as clipping mask). @param {CanvasRenderingContext2D} ctx @param {number} cx @param {number} cy @param {number} r */
export function clipCirc(ctx,cx,cy,r){ctx.beginPath();ctx.arc(cx,cy,Math.max(r,0.1),0,TAU);ctx.closePath();}
/** Fill a solid circle. @param {CanvasRenderingContext2D} ctx @param {number} cx @param {number} cy @param {number} r @param {string} f */
export function fillCirc(ctx,cx,cy,r,f){ctx.beginPath();ctx.arc(cx,cy,Math.max(r,0.1),0,TAU);ctx.fillStyle=f;ctx.fill();}
/** Sphere shading overlay. @param {CanvasRenderingContext2D} ctx @param {number} cx @param {number} cy @param {number} r */
...
/** Park-Miller LCG seeded random generator factory. @param {number} s @returns {() => number} */
export function seedR(s){var v=s;return function(){v=(v*16807)%2147483647;return(v-1)/2147483646;};}
/** Interpolate two CSS rgb triple strings. @param {string} a @param {string} b @param {number} f @returns {string} */
