...
var FL=[{k:"all",l:"全体"},{k:"sun",l:"太陽"},{k:"Mercury",l:"水星"},{k:"Venus",l:"金星"},{k:"Earth",l:"地球"},{k:"Mars",l:"火星"},{k:"Jupiter",l:"木星"},{k:"Saturn",l:"土星"},{k:"Uranus",l:"天王星"},{k:"Neptune",l:"海王星"},{k:"Halley",l:"ハレー彗星"},{k:"Encke",l:"エンケ彗星"}];
...
function drawLanding(ctx,W,H,t,plName,yaw,lat,fov){
  ...
  var pl=PL_MAP[plName];if(!pl)return;
  ...
  var descs={Mercury:"大気なし・灼熱の昼(430℃)と極寒の夜(−180℃)",Venus:"厚い硫酸雲・気温462℃・気圧90気圧",Earth:"青い空・白い雲・生命の惑星",Mars:"薄いCO₂大気・赤い空・砂嵐",Jupiter:"ガス惑星・巨大な雲の海",Saturn:"ガス惑星・空を横切る壮大なリング",Uranus:"氷の巨人・メタンの青い大気",Neptune:"最果ての惑星・時速2000kmの暴風"};
...
export default function App(){
  ...
  var S=useRef({t:dateToSimDays(new Date().toISOString().slice(0,10))||0,cam:{rx:-0.5,ry:0.3,zm:1,fx:0,fy:0,fz:0},dr:null,pi:null,trails:PL.map(function(){return[];}),hitAreas:[],dragged:false});
  ...
  var[touring,setTouring]=useState(false);var tourRef=useRef({active:false,idx:0,timer:0});
  ...
  var landR=useRef(null);var landYR=useRef(0);var landLatR=useRef(0);
  ...
  useEffect(function(){tourRef.current.active=touring;if(touring){tourRef.current.idx=0;tourRef.current.timer=0;}},[touring]);
  ...
  var zIn=useCallback(function(){setZi(function(p){var n=Math.min(ZS.length-1,p+1);dz(n);return n;});},[dz]);
  var zOut=useCallback(function(){setZi(function(p){var n=Math.max(0,p-1);dz(n);return n;});},[dz]);
  var tog=useCallback(function(k){setSh(function(p){var o={};for(var x in p)o[x]=p[x];o[k]=!p[k];return o;});},[]);
  var autoZoom=useCallback(function(name,isUni){if(!isUni)return;var wr;if(name==="sun"){wr=(SRR/1000)*DK;}else if(name==="all"){return;}else{var pl=PL_MAP[name];if(!pl)return;wr=(pl.r/1000)*DK;}var ideal=30/Math.max(wr,0.00001),best=0,bd=1e15;for(var i=0;i<ZS.length;i++){var d=Math.abs(ZS[i]-ideal);if(d<bd){bd=d;best=i;}}dz(best);ziR.current=best;setZi(best);},[dz]);
  useEffect(function(){if(uni&&foc!=="all"){autoZoom(foc,true);}},[uni,foc,autoZoom]);

  function findInfo(k){if(k==="sun")return{type:"sun"};var pl=PL_MAP[k];if(pl)return{type:"planet",pl:pl};var cm=COMET_MAP[k];if(cm)return{type:"comet",cm:cm};return null;}
  var stopTour=useCallback(function(){setTouring(false);tourRef.current.active=false;},[]);
  var focusOn=useCallback(function(k){
    stopTour();setFoc(k);autoZoom(k,uni);setInfo(k==="all"?null:findInfo(k));
    /* If currently landed, switch landing to new planet (or takeoff if not landable) */
    if(landR.current){if(PL_MAP[k]){setLanding(k);setLandYaw(0);setLandLat(0);setLandFov(1);}else{setLanding(null);}}
  },[stopTour,autoZoom,uni]);

  /* Central landing helper: stops tour, exits galaxy view, focuses planet, lands */
  var doLanding=useCallback(function(plName){
    if(!PL_MAP[plName])return;
    stopTour();
  ...
  var importState=useCallback(function(code){
    try{
      ...
      setLanding(null);stopTour();setCompare(false);
  ...
  useEffect(function(){
    ...
    function md(e){e.preventDefault();sim.dr={x:e.clientX,y:e.clientY};sim.dragged=false;}
    function mm(e){if(!sim.dr)return;var dx=e.clientX-sim.dr.x,dy=e.clientY-sim.dr.y;if(Math.abs(dx)+Math.abs(dy)>3)sim.dragged=true;if(landR.current){landYR.current+=dx*0.008;setLandYaw(landYR.current);landLatR.current=Math.max(-90,Math.min(90,landLatR.current-dy*0.3));setLandLat(landLatR.current);}else{sim.cam.ry+=dx*0.005;sim.cam.rx=Math.max(-1.5,Math.min(1.5,sim.cam.rx+dy*0.005));}sim.dr.x=e.clientX;sim.dr.y=e.clientY;}
    function mu(){sim.dr=null;}
    function wl(e){e.preventDefault();if(landR.current){var f=landFovR.current*(e.deltaY>0?1.1:0.9);f=Math.max(0.3,Math.min(3,f));landFovR.current=f;setLandFov(f);return;}var d2=e.deltaY>0?-1:1,c2=ziR.current,n=Math.max(0,Math.min(ZS.length-1,c2+d2));if(n!==c2){dz(n);ziR.current=n;setZi(n);}}
    ...
    function tst(e){if(e.touches.length===1){sim.dr={x:e.touches[0].clientX,y:e.touches[0].clientY};sim.dragged=false;}if(e.touches.length===2){sim.pi=td3(e);sim.dr=null;}}
    function tmv(e){e.preventDefault();if(e.touches.length===1&&sim.dr){var dx=e.touches[0].clientX-sim.dr.x,dy=e.touches[0].clientY-sim.dr.y;if(Math.abs(dx)+Math.abs(dy)>3)sim.dragged=true;if(landR.current){landYR.current+=dx*0.008;setLandYaw(landYR.current);landLatR.current=Math.max(-90,Math.min(90,landLatR.current-dy*0.3));setLandLat(landLatR.current);}else{sim.cam.ry+=dx*0.005;sim.cam.rx=Math.max(-1.5,Math.min(1.5,sim.cam.rx+dy*0.005));}sim.dr.x=e.touches[0].clientX;sim.dr.y=e.touches[0].clientY;}if(e.touches.length===2&&sim.pi){var d3=td3(e),ratio=d3/sim.pi;if(landR.current){/* Landing: pinch adjusts FOV (zoom binoculars) */var newFov=Math.max(0.3,Math.min(3,landFovR.current/ratio));landFovR.current=newFov;setLandFov(newFov);sim.pi=d3;}else if(ratio>1.06||ratio<0.94){var dir=ratio>1?1:-1,c3=ziR.current,n2=Math.max(0,Math.min(ZS.length-1,c3+dir));if(n2!==c3){dz(n2);ziR.current=n2;setZi(n2);}sim.pi=d3;}}}
    function ten(e){if(e.touches.length<2)sim.pi=null;if(e.touches.length===0)sim.dr=null;}
    function kd(e){var k=e.key;if(k===" "){e.preventDefault();setPaused(function(p){return!p;});}else if(k==="0"){focusOn("all");}else if(k.toLowerCase()==="s"){focusOn("sun");}else if(k>="1"&&k<="8"){focusOn(PL[parseInt(k)-1].n);}else if(k==="9"){focusOn("Halley");}else if(k.toLowerCase()==="e"){focusOn("Encke");}else if(k==="+"||k==="="){e.preventDefault();zIn();}else if(k==="-"||k==="_"){e.preventDefault();zOut();}else if(k==="ArrowRight"){var ci2=SP.indexOf(spR.current);if(ci2<SP.length-1){setSpd(SP[ci2+1]);setPaused(false);}}else if(k==="ArrowLeft"){var ci3=SP.indexOf(spR.current);if(ci3>0){setSpd(SP[ci3-1]);setPaused(false);}}else if(k.toLowerCase()==="c"){setCompare(function(p){return!p;});}else if(k.toLowerCase()==="t"){if(tourRef.current.active){stopTour();setFoc("all");setInfo(null);}else{setLanding(null);setTouring(true);setFoc("sun");setInfo({type:"sun"});}}else if(k.toLowerCase()==="m"){setBgm(function(p){return!p;});}
    ...
    cv.addEventListener("mousedown",md);window.addEventListener("mousemove",mm);window.addEventListener("mouseup",mu);cv.addEventListener("wheel",wl,{passive:false});cv.addEventListener("touchstart",tst,{passive:true});cv.addEventListener("touchmove",tmv,{passive:false});cv.addEventListener("touchend",ten);window.addEventListener("keydown",kd);
    ...
    function frame(ts2){
      ...
      var pd=[];for(var i=0;i<PL.length;i++){var pl=PL[i],oRv=oR(pl,_rd,_un),ang=(t/pl.p)*TAU;pd.push({pl:pl,oR:oRv,wx:Math.cos(ang)*oRv,wy:0,wz:Math.sin(ang)*oRv,vr:pRf(pl,_rp,_un),rotAng:(t/Math.abs(pl.rot))*TAU*(pl.rot<0?-1:1)});}
      ...
      trailTimer+=dt;if(trailTimer>0.05&&!pausR.current){trailTimer=0;for(var ti=0;ti<pd.length;ti++){sim.trails[ti].push({x:pd[ti].wx,z:pd[ti].wz});if(sim.trails[ti].length>TRAIL_LEN)sim.trails[ti].shift();}}
      ...
      for(var dr=0;dr<items.length;dr++){
        ...
        if(it.k==="s"){
          ...
          if(show.labels){ctx.fillStyle="rgba(255,220,100,0.85)";ctx.font="bold 11px sans-serif";ctx.textAlign="center";ctx.fillText("太陽",sunPj.x,sunPj.y-srScr-7);}
          ...
          if(show.labels){ctx.fillStyle="rgba(255,255,255,0.85)";ctx.font="10px sans-serif";ctx.textAlign="center";ctx.fillText(pdt.pl.j,ppp.x,ppp.y-rr-7);if(show.tilt){ctx.fillStyle="rgba(255,255,100,0.45)";ctx.font="8px sans-serif";ctx.fillText(pdt.pl.t+"°",ppp.x,ppp.y+rr+13);}}
        }
      }
      ...
      var tr2=tourRef.current;if(tr2.active){tr2.timer+=dt;if(tr2.timer>=TOUR_HOLD){tr2.timer=0;tr2.idx=(tr2.idx+1)%TOUR_SEQ.length;if(tr2.idx===0){tr2.active=false;setTouring(false);setFoc("all");setInfo(null);}else{var tk=TOUR_SEQ[tr2.idx];setFoc(tk);setInfo(findInfo(tk));}}var tProg=tr2.timer/TOUR_HOLD,tBarW=180,tBarX=(W-tBarW)/2,tBarY=H-28;ctx.fillStyle="rgba(8,10,20,0.7)";ctx.fillRect(tBarX-8,tBarY-14,tBarW+16,24);ctx.fillStyle="rgba(255,255,255,0.15)";ctx.fillRect(tBarX,tBarY,tBarW,4);ctx.fillStyle="rgba(100,180,255,0.7)";ctx.fillRect(tBarX,tBarY,tBarW*tProg,4);ctx.fillStyle="rgba(255,255,255,0.6)";ctx.font="10px sans-serif";ctx.textAlign="center";ctx.fillText("ツアー: "+TOUR_NAMES[tr2.idx]+" ("+(tr2.idx+1)+"/"+TOUR_SEQ.length+")",W/2,tBarY-3);}

      if(cmpR.current){ctx.fillStyle="rgba(3,3,12,0.92)";ctx.fillRect(0,0,W,H);ctx.fillStyle="rgba(255,255,255,0.5)";ctx.font="bold 13px sans-serif";ctx.textAlign="center";ctx.fillText("サイズ比較モード（実比率）",W/2,30);var cmpY=H*0.55,cmpScale=Math.min((H*0.35)/PL[4].r,(W*0.6)/(PL[4].r*2)),sunScr=SRR*cmpScale,sunCX=-sunScr+50;ctx.save();ctx.beginPath();ctx.rect(0,60,W,H-60);ctx.clip();clipCirc(ctx,sunCX,cmpY,sunScr);ctx.fillStyle="rgba(255,200,50,1)";ctx.fill();ctx.restore();ctx.fillStyle="rgba(255,220,100,0.7)";ctx.font="10px sans-serif";ctx.textAlign="left";ctx.fillText("太陽 ☀",58,cmpY-8);var gap=(W*0.75)/PL.length;for(var cpi=0;cpi<PL.length;cpi++){var cpx=W*0.18+cpi*gap,cpr=Math.max(PL[cpi].r*cmpScale,1.5);drawPlanetBody(ctx,cpx,cmpY,cpr,PL[cpi],t/Math.abs(PL[cpi].rot)*TAU);if(PL[cpi].n==="Saturn")dRi(ctx,cpx,cmpY,0,cpr,{fx:0,fy:0,fz:0,rx:0,ry:0,zm:1},PL[cpi].t);ctx.fillStyle="rgba(255,255,255,0.8)";ctx.font="10px sans-serif";ctx.textAlign="center";ctx.fillText(PL[cpi].j,cpx,cmpY+cpr+18);ctx.fillStyle="rgba(255,255,255,0.4)";ctx.font="8px sans-serif";ctx.fillText("地球の"+(PL[cpi].r/6.4).toFixed(1)+"倍",cpx,cmpY+cpr+30);}}
  ...
  return(
    <div style={{width:"100%",height:"100vh",background:"rgba(3,3,10,1)",position:"relative",overflow:"hidden"}}>
      <canvas ref={cR} style={{display:"block",width:"100%",height:"100%",touchAction:"none",cursor:"crosshair"}} onClick={handleClick}/>

      {/* Focus panel */}
      {cleanView===0&&<DragPanel style={Object.assign({},pn,{top:10,left:10,maxWidth:300})}><div style={lb}>フォーカス ⠿</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{FL.map(function(f){return <button key={f.k} style={foc===f.k?bN:bF} onClick={function(){focusOn(f.k);}}>{f.l}</button>;})}</div></DragPanel>}
      ...
      {cleanView===0&&<DragPanel style={Object.assign({},pn,{bottom:10,left:10,maxWidth:300})}>
        ...
        <div style={{marginTop:6,display:"flex",gap:3,flexWrap:"wrap"}}><button style={uni?bU:bF} onClick={function(){setUni(function(p){return!p;});}}>統一比率{uni?" ●":""}</button><button style={compare?bT("100,220,150"):bF} onClick={function(){setCompare(function(p){return!p;});}}>比較{compare?" ●":""}</button><button style={touring?bT("200,100,255"):bF} onClick={function(){if(touring){stopTour();setFoc("all");setInfo(null);}else{setLanding(null);setTouring(true);setFoc("sun");setInfo({type:"sun"});}}}>{touring?"ツアー停止":"ツアー"}</button><button style={bgm?bT("80,200,220"):bF} onClick={function(){setBgm(function(p){return!p;});}}>BGM{bgm?" ♪":""}</button></div>
