...
export default function App(){
  ...
  var[panels,dispatchPanel]=useReducer(panelReducer,Object.assign({},PANEL_INIT,typeof window!=="undefined"&&(window.innerWidth<640||window.innerHeight<500)?{dispColl:true}:{}));var eventsRef=useRef([]);
  ...
  useEffect(function(){
    ...
    function frame(ts2){
      ...
      var pd=[];for(var i=0;i<allBodies.length;i++){var pl=allBodies[i],oRv=oR(pl,_rd,_un),os=orbitState(pl,t),orr=oRv*os.rf;pd.push({pl:pl,oR:oRv,wx:Math.cos(os.theta)*orr,wy:0,wz:Math.sin(os.theta)*orr,vr:pRf(pl,_rp,_un),rotAng:(t/Math.abs(pl.rot))*TAU*(pl.rot<0?-1:1)});}
      ...
      var cd=[];for(var cci=0;cci<COMETS.length;cci++){var cm0=COMETS[cci],cm0E=cm0.ecc;var cm0OrbR=_rd||_un?cm0.a*DK:(160+Math.pow((cm0.a-228)/4267,0.55)*280);var cm0M=((t/cm0.p)+cm0.phase0)*TAU;var cm0V=solveKepler(cm0M,cm0E).nu;var cm0R=cm0OrbR*(1-cm0E*cm0E)/(1+cm0E*Math.cos(cm0V));cd.push({cm:cm0,orbR:cm0OrbR,wx:Math.cos(cm0V+cm0.inc)*cm0R,wy:0,wz:Math.sin(cm0V+cm0.inc)*cm0R});}
      ...
      if(show.trails){for(var tri=0;tri<sim.trails.length;tri++){var trail=sim.trails[tri];if(trail.length<3)continue;var _plT=pd[tri].pl;var cStr=_plT.c.replace(",1)","");var _rgb=cStr.replace("rgba(","").split(",");var _tR=parseInt(_rgb[0]),_tG=parseInt(_rgb[1]),_tB=parseInt(_rgb[2]);var _plE=_plT.ecc||0,_plD=_plT.d;var _ra=_plD*(1+_plE),_rpA=_plD*(1-_plE),_rRange=_ra-_rpA;var bs=Math.max(2,Math.floor(trail.length/10));for(var tb=0;tb<trail.length-1;tb+=bs){var te2=Math.min(tb+bs+1,trail.length),mA=((tb+te2)*0.5/trail.length)*0.5;var _ms=trail[Math.floor((tb+te2)/2)];var _mr=Math.sqrt(_ms.x*_ms.x+_ms.z*_ms.z);var _sf=_rRange>0?Math.max(0,Math.min(1,(_ra-_mr)/_rRange)):0;var _aF=Math.min(0.78,mA*(1+_sf*0.9));var _Rn=Math.round(_tR+(255-_tR)*_sf*0.52);var _Gn=Math.round(_tG+(195-_tG)*_sf*0.40);var _Bn=Math.round(_tB+(65-_tB)*_sf*0.32);ctx.beginPath();ctx.strokeStyle="rgba("+_Rn+","+_Gn+","+_Bn+","+_aF.toFixed(2)+")";ctx.lineWidth=1.2+_sf*1.0;var fp=pj(trail[tb].x,0,trail[tb].z,cam);ctx.moveTo(fp.x,fp.y);for(var tj=tb+1;tj<te2;tj++){var cp=pj(trail[tj].x,0,trail[tj].z,cam);ctx.lineTo(cp.x,cp.y);}ctx.stroke();}}}
      ...
      /* Comets */
      for(var cmi=0;cmi<cd.length;cmi++){var cm=cd[cmi].cm,cmE=cm.ecc,cmOrbR=cd[cmi].orbR;var cmPj=pj(cd[cmi].wx,0,cd[cmi].wz,cam);var cmH=Math.max(cm.sz,0.8),cmZm=Math.pow(cam.zm,0.3);var tdx=cmPj.x-sunPj.x,tdy=cmPj.y-sunPj.y,tl2=Math.sqrt(tdx*tdx+tdy*tdy);if(tl2<0.1){tdx=1;tdy=0;tl2=1;}var tnx=tdx/tl2,tny=tdy/tl2,tpx=-tny,tpy=tnx;var csd=Math.sqrt(cd[cmi].wx*cd[cmi].wx+cd[cmi].wz*cd[cmi].wz),peri=cmOrbR*(1-cmE),tI=Math.max(0.1,Math.min(1.2,peri*3/Math.max(csd,0.1))),tLen=cm.tailLen*tI*cmZm;
      ...
      for(var dr=0;dr<items.length;dr++){
        ...
        if(it.k==="s"){
          ...
          if(pdt.pl.n==="Saturn"&&rr>6)dRingShadow(ctx,pdt.wx,pdt.wy,pdt.wz,pdt.vr,rr,ppp.x,ppp.y,cam,pdt.pl.t);
      ...
      /* Orbital period progress ring — shown around focused planet/dwarf */
      if(fc!=="all"&&fc!=="sun"&&!cmpR.current&&!landR.current){ctx.setLineDash([]);for(var pri=0;pri<pd.length;pri++){if(pd[pri].pl.n!==fc)continue;var _pringPP=pjArr[pri],_pringRr=Math.max(pd[pri].vr*cam.zm,1.5);var _pringR=Math.max(12,_pringRr+8);var _pringF=((t/pd[pri].pl.p)%1+1)%1;var _pringCol=pd[pri].pl.c.replace(",1)","");ctx.globalAlpha=0.18;ctx.strokeStyle=_pringCol+",0.6)";ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(_pringPP.x,_pringPP.y,_pringR,0,TAU);ctx.stroke();var _pringEnd=-Math.PI*0.5+_pringF*TAU;ctx.globalAlpha=0.75;ctx.strokeStyle=_pringCol+",1)";ctx.lineWidth=2;ctx.beginPath();ctx.arc(_pringPP.x,_pringPP.y,_pringR,-Math.PI*0.5,_pringEnd);ctx.stroke();ctx.globalAlpha=1;ctx.fillStyle=pd[pri].pl.c;ctx.beginPath();ctx.arc(_pringPP.x+Math.cos(_pringEnd)*_pringR,_pringPP.y+Math.sin(_pringEnd)*_pringR,2.5,0,TAU);ctx.fill();var _pringStr=pd[pri].pl.p<400?Math.round(pd[pri].pl.p)+"日":(pd[pri].pl.p/365.25).toFixed(2)+"年";ctx.globalAlpha=0.65;ctx.fillStyle=_pringCol+",0.85)";ctx.font="8px sans-serif";ctx.textAlign="center";ctx.fillText(_pringStr+"  "+Math.round(_pringF*100)+"%",_pringPP.x,_pringPP.y+_pringR+11);ctx.globalAlpha=1;break;}}
      ...
      if(show.orbits){for(var oi2=0;oi2<pd.length;oi2++){var osr2=pd[oi2].oR*cam.zm;if(osr2<0.5||osr2>orbMaxR||sd2>=osr2*osr2)continue;dOb(ctx,pd[oi2].oR,cam,pd[oi2].pl.cRGB,true,pd[oi2].pl.ecc,pd[oi2].pl.peri);}}
  ...
  return(
    <div style={{width:"100%",height:"100dvh",background:"rgba(3,3,10,1)",position:"relative",overflow:"hidden"}}>
      ...
      {landing&&<div style={{position:"absolute",top:10,right:10,zIndex:26,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
        <div style={{background:"rgba(0,5,18,0.82)",border:"1px solid rgba(100,160,255,0.2)",borderRadius:6,padding:"6px 8px",display:"flex",flexWrap:"wrap",justifyContent:"flex-end",gap:3,maxWidth:isPhone?"calc(100vw - 20px)":260}}>
          <span style={{color:"rgba(180,210,255,0.7)",fontSize:9,width:"100%",marginBottom:2}}>{lang==="en"?"Speed":"速度"}</span>
          <button aria-label={paused?(lang==="en"?"Play":"再生"):(lang==="en"?"Pause":"一時停止")} style={Object.assign({},paused?bT("100,180,255"):bF,isPhone?{}:{padding:"2px 6px",fontSize:10})} onClick={function(){setPaused(function(p){return!p;})}}>{paused?"▶":"⏸"}</button>
          {LAND_SP.map(function(s){return <button key={s.l} style={Object.assign({},landSpd===s.v&&!paused?bN:bF,isPhone?{}:{padding:"2px 5px",fontSize:9})} onClick={function(){setLandSpd(s.v);landSpdR.current=s.v;setPaused(false);}}>{s.l}</button>;})}
          ...
          <button style={Object.assign({},bT("100,230,160"),isPhone?{marginTop:2}:{padding:"2px 6px",fontSize:9,marginTop:2,width:"100%"})} onClick={function(){
