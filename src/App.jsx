...
export default function App(){
  ...
  var importState=useCallback(function(code){
    try{
      ...
      setLanding(null);setCompare(false);
  ...
  useEffect(function(){
    ...
    function tst(e){if(cmpR.current){if(e.touches.length===1){sim.cmpDrag={x:e.touches[0].clientX};sim.dragged=false;}if(e.touches.length===2){sim.cmpPinch=td3(e);sim.cmpDrag=null;}return;}if(e.touches.length===1){sim.dr={x:e.touches[0].clientX,y:e.touches[0].clientY};sim.dragged=false;}if(e.touches.length===2){sim.pi=td3(e);sim.dr=null;}}
    ...
    function frame(ts2){
      ...
      if(cmpR.current){var cmpSt=cmpStateRef.current;ctx.fillStyle="rgba(3,3,12,0.92)";ctx.fillRect(0,0,W,H);ctx.fillStyle="rgba(255,255,255,0.5)";ctx.font="bold 13px sans-serif";ctx.textAlign="center";ctx.fillText("サイズ比較モード（実比率）",W/2,30);var cmpY=H*0.55,cmpScale=Math.min((H*0.35)/PL[4].r,(W*0.5)/(PL[4].r*2))*cmpSt.zm,sunScr=SRR*cmpScale,sunCX=-sunScr+50;ctx.save();ctx.beginPath();ctx.rect(0,60,W,H-60);ctx.clip();clipCirc(ctx,sunCX,cmpY,sunScr);ctx.fillStyle="rgba(255,200,50,1)";ctx.fill();ctx.fillStyle="rgba(255,220,100,0.7)";ctx.font="10px sans-serif";ctx.textAlign="left";ctx.fillText("太陽 ☀",Math.max(4,sunCX+sunScr+4),cmpY-8);var allCmp=PL.concat(DWARFS);var cmpGap=Math.max(50*cmpSt.zm,(W*0.85)/allCmp.length);for(var cpi=0;cpi<allCmp.length;cpi++){var cpx=W*0.12+cpi*cmpGap+cmpSt.offX,cpr=Math.max(allCmp[cpi].r*cmpScale,1.5);if(cpx+cpr<0||cpx-cpr>W)continue;drawPlanetBody(ctx,cpx,cmpY,cpr,allCmp[cpi],t/Math.abs(allCmp[cpi].rot)*TAU);if(allCmp[cpi].n==="Saturn")dRi(ctx,cpx,cmpY,0,cpr,{fx:0,fy:0,fz:0,rx:0,ry:0,zm:1},allCmp[cpi].t);ctx.fillStyle="rgba(255,255,255,0.8)";ctx.font="10px sans-serif";ctx.textAlign="center";ctx.fillText(allCmp[cpi].j,cpx,cmpY+cpr+18);ctx.fillStyle="rgba(255,255,255,0.4)";ctx.font="8px sans-serif";ctx.fillText("地球の"+(allCmp[cpi].r/6.4).toFixed(1)+"倍",cpx,cmpY+cpr+30);}ctx.restore();ctx.fillStyle="rgba(255,255,255,0.2)";ctx.font="10px sans-serif";ctx.textAlign="center";ctx.fillText("← ドラッグでスクロール　ピンチ/ホイールでズーム →",W/2,H-15);}
