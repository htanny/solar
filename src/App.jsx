...
function dOb(ctx,rad,cam,col){
  ...
  ctx.stroke();
  /* Front arc (z<0, near viewer): brighter solid */
...
export default function App(){
  ...
  var S=useRef({t:dateToSimDays(new Date().toISOString().slice(0,10))||0,cam:{rx:-0.55,ry:0.4,zm:1,tzm:1,fx:0,fy:0,fz:0},dr:null,pi:null,trails:PL.map(function(){return[];}),hitAreas:[],dragged:false});
  ...
  useEffect(function(){
    ...
    function frame(ts2){
      ...
      if(show.orbits){for(var oi=0;oi<pd.length;oi++){var _oc=pd[oi].pl.c.match(/(\d+),(\d+),(\d+)/);dOb(ctx,pd[oi].oR,cam,_oc?_oc[1]+","+_oc[2]+","+_oc[3]:null);}}
  ...
  return(
    <div style={{width:"100%",height:"100dvh",background:"rgba(3,3,10,1)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",top:4,left:4,color:"rgba(255,255,255,0.35)",fontSize:9,fontFamily:"system-ui,sans-serif",pointerEvents:"none",zIndex:20}}>v2.1.5</div>
