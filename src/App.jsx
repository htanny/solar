...
export default function App(){
  ...
  var startQuiz=useCallback(function(){var qs=QUIZ_DATA.slice().sort(function(){return Math.random()-0.5;}).slice(0,5);setQuizState({questions:qs,idx:0,score:0,answered:null});},[]);
  ...
  return(
    <div style={{width:"100%",height:"100dvh",background:"rgba(3,3,10,1)",position:"relative",overflow:"hidden"}}>
      ...
      {cleanView===0&&!landing&&<DragPanel style={Object.assign({},pn,{bottom:10,left:10,maxWidth:300})}>
        <div style={Object.assign({},lb,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2})}><span>表示 ⠿</span><button style={Object.assign({},bF,{fontSize:9,padding:"1px 6px",lineHeight:1.4})} onClick={function(){dispatchPanel({type:"TOGGLE",key:"dispColl"});}}>{panels.dispColl?"▼":"▲"}</button></div><div style={{display:panels.dispColl?"none":"block"}}><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{[{k:"orbits",l:"軌道"},{k:"trails",l:"軌跡"},{k:"belt",l:"小惑星帯"},{k:"nasteroid",l:"準惑星名"},{k:"tilt",l:"地軸"},{k:"moon",l:"月"},{k:"labels",l:"ラベル"},{k:"planets",l:"惑星"},{k:"lagrange",l:"L点"},{k:"spacecraft",l:"探査機"},{k:"cme",l:"CME"},{k:"distbar",l:"距離バー"}].map(function(x){return <button key={x.k} style={sh[x.k]?bN:bF} onClick={function(){tog(x.k);}}>{x.l}</button>;})}</div>
        ...
        <div style={{marginTop:6,display:"flex",gap:3,flexWrap:"wrap"}}><button style={uni?bU:bF} onClick={function(){setUni(function(p){return!p;});}}>統一比率{uni?" ●":""}</button><button style={compare?bT("100,220,150"):bF} onClick={function(){setCompare(function(p){if(!p)cmpStateRef.current={offX:0,zm:1};return!p;});}}>比較{compare?" ●":""}</button><button style={touring?bT("200,100,255"):bF} onClick={function(){if(touring){stopTour();setFoc("all");setInfo(null);}else{setLanding(null);stopTour();setTouring(true);tourRef.current={active:true,idx:0,timer:0,trans:false};setFoc("sun");setInfo({type:"sun"});}}}>{touring?"ツアー停止":"学習ツアー"}</button><button style={bgm?bT("80,200,220"):bF} onClick={function(){setBgm(function(p){return!p;});}}>BGM{bgm?" ♪":""}</button><button style={lang==="en"?bT("100,220,180"):bF} onClick={function(){setLang(function(p){return p==="ja"?"en":"ja";});}}>EN/JA</button><button style={measureMode?bT("255,180,80"):bF} onClick={function(){setMeasureMode(function(p){if(p)setMeasurePair([]);return!p;});}}>{measureMode?"計測中"+(measurePair.length===0?"(1つ目)":measurePair.length===1?"(2つ目)":""):"📐計測"}</button></div>
        <div style={Object.assign({},lb,{marginTop:8,marginBottom:4})}>ツール</div>
        <div style={{display:"flex",gap:3,flexWrap:"wrap",alignItems:"center"}}>
          <button style={panels.showDate?bN:bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"showDate"});}}>日付移動</button>
          <button style={bF} onClick={takeScreenshot}>📷 撮影モード</button>
          <button style={bF} onClick={shareURL}>🔗 共有</button>
          <button style={bF} onClick={function(){dispatchPanel({type:"SET",key:"importMode",value:true});}}>📥 読込</button>
          ...
          <button style={panels.showEvents?bT("255,200,80"):bF} onClick={function(){if(!panels.showEvents){eventsRef.current=scanEvents(S.current.t);}dispatchPanel({type:"TOGGLE",key:"showEvents"});}}>📅 天文イベント</button>
          <button style={panels.searchOpen?bT("100,210,255"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"searchOpen"});setSearchQ("");}}>🔍 検索</button>
          <button style={panels.exoOpen?bT("255,150,90"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"exoOpen"});}}>🪐 系外惑星</button>
          <button style={panels.nightSkyOpen?bT("255,220,80"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"nightSkyOpen"});}}>🌙 今夜の空</button>
          <button style={panels.bookOpen?bT("255,220,120"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"bookOpen"});setBookmarkName("");}}>🔖 ブックマーク</button>
          <button style={bF} onClick={function(){setOnboardStep(0);}}>❓ ガイド</button>
          <button style={quizState?bT("255,200,80"):bF} onClick={function(){if(quizState)closeQuiz();else startQuiz();}}>🎯 {quizState?"クイズ終了":"クイズ"}</button>
          <button style={panels.compareTable?bT("180,255,200"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"compareTable"});}} title="全惑星・準惑星の物理量比較表">📊 比較表</button>
        </div>
        <div style={Object.assign({},lb,{marginTop:6,marginBottom:2})}>可視化</div>
        <div style={Object.assign({},lb,{marginTop:2,marginBottom:2,color:"rgba(255,255,255,0.25)",fontSize:8})}>宇宙構造</div>
        ...
        <div style={Object.assign({},lb,{marginBottom:2,color:"rgba(255,255,255,0.25)",fontSize:8})}>物理現象</div>
        <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:4}}>
          <button style={magneto?bN:bF} onClick={function(){setMagneto(function(p){return!p;});}} title="地球磁気圏・ヴァン・アレン帯">🌐 磁気圏</button>
          <button style={hillSphere?bN:bF} onClick={function(){setHillSphere(function(p){return!p;});}} title="ヒル球（重力圏）">⭕ ヒル球</button>
          <button style={shadowCone?bN:bF} onClick={function(){setShadowCone(function(p){return!p;});}} title="日食影錐（日食中のみ）">🌑 影錐</button>
          <button style={tidalForce?bN:bF} onClick={function(){setTidalForce(function(p){return!p;});}} title="地球への潮汐力ベクトル">🌊 潮汐力</button>
        </div>
        <div style={Object.assign({},lb,{marginBottom:2,color:"rgba(255,255,255,0.25)",fontSize:8})}>観測ツール</div>
        <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
          <button style={telescopeMode?bT("100,220,150"):bF} onClick={function(){setTelescopeMode(function(p){return!p;});}} title="望遠鏡モード">🔭 望遠鏡</button>
          <button style={panels.moonCal?bT("200,180,255"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"moonCal"});}} title="月相カレンダー">🌙 月相</button>
          <button style={panels.orbElemOpen?bT("180,220,255"):bF} onClick={function(){dispatchPanel({type:"TOGGLE",key:"orbElemOpen"});}} title="軌道要素（惑星選択時）">📊 軌道</button>
      ...
      {panels.compareTable&&cleanView===0&&!landing&&<DragPanel style={Object.assign({},pn,{top:10,left:"50%",transform:"translateX(-50%)",width:"min(700px,calc(100vw - 20px))",maxHeight:"80vh",padding:"10px 12px",zIndex:25,overflowY:"auto"})}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><span style={{fontSize:12,fontWeight:"bold",color:"rgba(180,255,200,0.95)"}}>📊 惑星比較表</span><button style={Object.assign({},bF,{padding:"2px 6px",fontSize:9})} onClick={function(){dispatchPanel({type:"TOGGLE",key:"compareTable"});}}>✕</button></div>{(function(){var rows=PL.concat([MOON_INFO]).concat(DWARFS);rows=rows.slice().sort(function(a,b){var col=cmpSort.col,va,vb;if(col==="r"){va=a.r;vb=b.r;}else if(col==="d"){va=a.d;vb=b.d;}else if(col==="dens"){va=parseFloat(a.dens||0);vb=parseFloat(b.dens||0);}else if(col==="grav"){va=parseFloat(a.grav||0);vb=parseFloat(b.grav||0);}else if(col==="esc"){va=parseFloat(a.esc||0);vb=parseFloat(b.esc||0);}else if(col==="moons"){va=a.moons;vb=b.moons;}else if(col==="alb"){va=a.alb||0;vb=b.alb||0;}else{va=a.d;vb=b.d;}return cmpSort.asc?va-vb:vb-va;});var thS={fontSize:9,color:"rgba(180,220,255,0.8)",padding:"4px 6px",textAlign:"right",cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.1)",whiteSpace:"nowrap",userSelect:"none"};var tdS={fontSize:9,color:"rgba(255,255,255,0.75)",padding:"4px 6px",textAlign:"right",borderBottom:"1px solid rgba(255,255,255,0.05)"};var tdN={fontSize:9,color:"rgba(255,255,255,0.95)",padding:"4px 6px",textAlign:"left",borderBottom:"1px solid rgba(255,255,255,0.05)",fontWeight:"bold"};var mkTh=function(label,col){var isActive=cmpSort.col===col;return <th key={col} style={Object.assign({},thS,isActive?{color:"rgba(255,220,80,1)"}:{})} onClick={function(){setCmpSort(function(p){return p.col===col?{col:col,asc:!p.asc}:{col:col,asc:true};});}}>{label}{isActive?(cmpSort.asc?"▲":"▼"):""}</th>;};return <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:9}}><thead><tr><th style={Object.assign({},thS,{textAlign:"left"})}>天体</th>{mkTh("距離(Mkm)","d")}{mkTh("半径(km)","r")}{mkTh("密度(g/cm³)","dens")}{mkTh("重力(m/s²)","grav")}{mkTh("脱出(km/s)","esc")}{mkTh("衛星","moons")}{mkTh("アルベド","alb")}</tr></thead><tbody>{rows.map(function(pl){var isEarth=pl.n==="Earth";var isFoc=foc===pl.n;var rKm=Math.round(pl.r*1000);var rEarth=(pl.r/6.371).toFixed(2);var gRaw=parseFloat(pl.grav||0);var gEarth=(gRaw/9.8).toFixed(2);var rowStyle={background:isFoc?"rgba(100,180,255,0.12)":isEarth?"rgba(60,120,200,0.08)":"transparent"};return <tr key={pl.n} style={rowStyle} onClick={function(){focusOn(pl.n);}} title="クリックでフォーカス"><td style={tdN}><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:pl.c,marginRight:4,flexShrink:0}}></span>{lang==="en"?pl.n:pl.j}</td><td style={tdS}>{pl.n==="Moon"?"(0.384万km)":pl.d.toLocaleString()}</td><td style={tdS}>{rKm.toLocaleString()}<span style={{color:"rgba(255,255,255,0.35)",fontSize:8}}> ({rEarth}×)</span></td><td style={Object.assign({},tdS,pl.dens==="0.69"?{color:"rgba(255,220,80,1)"}:{})}>{pl.dens||"—"}</td><td style={tdS}>{pl.grav}<span style={{color:"rgba(255,255,255,0.35)",fontSize:8}}> ({gEarth}×)</span></td><td style={tdS}>{pl.esc||"—"}</td><td style={tdS}>{pl.moons}</td><td style={tdS}>{pl.alb!=null?(pl.alb*100).toFixed(0)+"%":"—"}</td></tr>;})}</tbody></table><div style={{marginTop:6,fontSize:8,color:"rgba(255,255,255,0.3)",textAlign:"right"}}>クリックで天体にフォーカス　ヘッダーでソート　地球比(×)</div></div>;})()}</DragPanel>}

      {quizState!==null&&cleanView===0&&!landing&&<DragPanel style={Object.assign({},pn,{bottom:60,left:"50%",transform:"translateX(-50%)",width:320,maxWidth:"calc(100vw - 20px)",padding:"14px 16px",zIndex:30})}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:12,fontWeight:"bold",color:"rgba(255,220,80,0.95)"}}>🎯 天体クイズ ({quizState.idx+1}/{quizState.questions.length})</span><button style={Object.assign({},bF,{padding:"2px 6px",fontSize:9})} onClick={closeQuiz}>✕</button></div>{quizState.idx<quizState.questions.length?(function(){var q=quizState.questions[quizState.idx];return <div><div style={{fontSize:11,color:"rgba(255,255,255,0.92)",lineHeight:"17px",marginBottom:10}}>{q.q}</div><div style={{display:"flex",flexDirection:"column",gap:5}}>{q.a.map(function(ans,ai){var isAnswered=quizState.answered!==null;var isCorrect=ai===q.c;var isChosen=ai===quizState.answered;var btnStyle=Object.assign({},bF,{textAlign:"left",padding:"6px 10px",fontSize:10,background:isAnswered?(isCorrect?"rgba(80,200,100,0.35)":isChosen?"rgba(255,80,80,0.3)":"rgba(255,255,255,0.04)"):"rgba(255,255,255,0.06)",border:isAnswered?(isCorrect?"1px solid rgba(80,200,100,0.6)":isChosen?"1px solid rgba(255,80,80,0.5)":"1px solid rgba(255,255,255,0.08)"):"1px solid rgba(255,255,255,0.15)",cursor:isAnswered?"default":"pointer"});return <button key={ai} style={btnStyle} onClick={function(){if(quizState.answered!==null)return;setQuizState(function(p){return Object.assign({},p,{answered:ai,score:ai===q.c?p.score+1:p.score});});}}>{ans}</button>;})}</div>{quizState.answered!==null&&<><div style={{marginTop:8,fontSize:9,color:"rgba(150,220,180,0.85)",lineHeight:"14px"}}>💡 {q.hint}</div><button style={Object.assign({},bT("100,180,255"),{marginTop:8,width:"100%",fontSize:11,padding:"6px"})} onClick={function(){setQuizState(function(p){var next=p.idx+1;return Object.assign({},p,{idx:next,answered:null});});}}>次の問題 →</button></>}</div>;}()):<div style={{textAlign:"center"}}><div style={{fontSize:22,marginBottom:8}}>{"⭐".repeat(quizState.score)+"☆".repeat(quizState.questions.length-quizState.score)}</div><div style={{fontSize:14,color:"rgba(255,220,80,1)",fontWeight:"bold",marginBottom:6}}>{quizState.score}/{quizState.questions.length} 正解</div><div style={{fontSize:10,color:"rgba(255,255,255,0.6)",marginBottom:12}}>{quizState.score===quizState.questions.length?"完璧！地学マスター🏆":quizState.score>=3?"よくできました！⭐":"もう一度チャレンジ！"}</div><div style={{display:"flex",gap:6}}><button style={Object.assign({},bT("100,180,255"),{flex:1,fontSize:11,padding:"7px"})} onClick={startQuiz}>もう一度</button><button style={Object.assign({},bF,{flex:1,fontSize:11,padding:"7px"})} onClick={closeQuiz}>閉じる</button></div></div>}</DragPanel>}
      ...
      {onboardStep>=0&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.78)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif"}} onClick={function(e){e.stopPropagation();}}>
        <div style={{background:"rgba(10,14,28,0.98)",border:"1px solid rgba(100,180,255,0.3)",borderRadius:16,padding:"22px 26px",maxWidth:350,width:"90%",color:"rgba(255,255,255,0.92)"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:20}}>
            ...
            <button style={bN} onClick={function(){var n=onboardStep+1;if(n>=5){setOnboardStep(-1);localStorage.setItem("solar_ob","1");setLanding(null);setTouring(true);tourRef.current={active:true,idx:0,timer:0,trans:false};setFoc("sun");setInfo({type:"sun"});}else setOnboardStep(n);}}>{onboardStep===4?"学習ツアーを開始 🚀":"次へ →"}</button>
          </div>
        </div>
      </div>}
    </div>
