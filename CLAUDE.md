...
- 天文クイズ（初級/中級/上級、計30問、EN/JA対応、選択肢シャッフル。受験対策の演習はExamPanelのドリルに一元化し、難易度選択画面から案内リンクで誘導）
- 受験対策パネル（`ExamPanel`）— 中学受験・中3理科向け「月の満ち欠け」「金星の見え方」「星の動き（日周・年周運動）」のインタラクティブ図解。月・金星は8位置クリックで地球から見た形・出入り時刻・入試の鉄則を表示。時刻スライダー付き「地上から見た空」ビュー（南向き地平線に月の方角・太陽位置・空の色を時刻連動で表示、金星は明け方東/夕方西の薄明ビュー）。星の動きタブは南の空（オリオン座/さそり座を月+時刻スライダーで動かし日周15°/h・年周30°/月を体感）と北の空（北極星中心の反時計回り、カシオペヤ座+北斗七星）の2ビュー。ミニドリルタブ（図解と同じデータから月・金星・星の計12パターン14候補を動的生成し毎回8問を抽選、誤答時は該当の図+時刻へワンタップジャンプ、通算成績を`solar_drill`に記録、結果をWeb Share API/クリップボードで共有可能）
...
- 左下操作パネルは用途別8セクション構成（表示トグル/実スケール/ビューモード/学習/観測・データ/ツール/可視化オーバーレイ/設定フッター）— ボタン追加時は該当セクションに配置すること
- PWAインストール促進バナー
